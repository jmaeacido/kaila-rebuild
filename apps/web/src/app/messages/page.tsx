...
import { BriefcaseBusiness, ChevronRight, ClipboardList, Home, MessageCircle, Plus, Search, ShieldCheck, UserRound } from "lucide-react";
...
export default function MessagesPage() {
  ...
  </div><nav className={styles.bottomNav} aria-label="Marketplace navigation"><Link href="/home"><Home aria-hidden="true" />Home</Link><Link href={isProvider ? "/opportunities" : "/home#current-title"}>{isProvider ? <Search aria-hidden="true" /> : <ClipboardList aria-hidden="true" />}{isProvider ? "Find work" : "Jobs"}</Link><Link href={isProvider ? "/home#current-title" : "/post-job"}>{isProvider ? <BriefcaseBusiness aria-hidden="true" /> : <Plus aria-hidden="true" />}{isProvider ? "Work" : "Post"}</Link><Link aria-current="page" href="/messages"><MessageCircle aria-hidden="true" />Messages</Link><Link href="/account"><UserRound aria-hidden="true" />Profile</Link></nav></main>;
}

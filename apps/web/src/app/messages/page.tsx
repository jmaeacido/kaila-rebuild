...
export default function MessagesPage() {
  ...
  </div><nav className={styles.bottomNav} aria-label="Marketplace navigation"><Link href="/home"><Home aria-hidden="true" />Home</Link><Link href={isProvider ? "/opportunities" : "/home#current-title"}>{isProvider ? <BriefcaseBusiness aria-hidden="true" /> : <ClipboardList aria-hidden="true" />}{isProvider ? "Work" : "Jobs"}</Link><Link href={isProvider ? "/opportunities" : "/post-job"}><Plus aria-hidden="true" />{isProvider ? "Find work" : "Post"}</Link><Link aria-current="page" href="/messages"><MessageCircle aria-hidden="true" />Messages</Link><Link href="/account"><UserRound aria-hidden="true" />Profile</Link></nav></main>;
}

...
type Opportunity = { id:number; jobId:string; state:string; title:string; description:string; client:{displayName:string;avatarUrl:string|null;rating:string|null;reviewCount:number}; area:{name:string}; category:{name:string;icon:string}; scheduleType:string; scheduledAt:string|null; budgetMinCentavos:number|null; budgetMaxCentavos:number|null; approximateAddress:string; approximateLocation:{latitude:number;longitude:number}|null; attachmentCount:number; offer:{id:string;status:string;latestRevisionNumber:number}|null };
...
function reputation(rating:string|null,count:number){return rating?`${Number(rating).toFixed(1)} · ${count} review${count===1?"":"s"}`:"New · No reviews yet";}

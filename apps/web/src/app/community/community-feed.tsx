...
import { CommunityFeedContext, CommunityPost, csrfFetch, fetchCommunityFeed, kindLabels, mapPublicCommunityPost, normalizeCommunityPost } from "./community-client";
import { useRealtimeInvalidation } from "../use-realtime-invalidation";
import styles from "./community.module.css";

export function CommunityFeed({ initialPosts }: { initialPosts: PublicCommunityPost[] }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const tag = searchParams.get("tag")?.toLowerCase() ?? "";
  const { isProvider } = useMarketplaceMode();
  const [posts, setPosts] = useState<CommunityPost[]>(() => initialPosts.map(mapPublicCommunityPost));
  const [feedContext, setFeedContext] = useState<CommunityFeedContext | null>(null);
  const [contextReady, setContextReady] = useState(false);
  const [status, setStatus] = useState<"loading" | "ready" | "error">(initialPosts.length > 0 ? "ready" : "loading");
  ...
  const load = useCallback(async (append = false) => {
    try {
      ...
      const response = await fetchCommunityFeed(query);
      if (!response.ok) throw new Error();
      const result = await response.json() as { data: Array<CommunityPost | PublicCommunityPost>; meta: { nextCursor: string | null } };
      setPosts((current) => (append ? [...current, ...result.data.map(normalizeCommunityPost)] : result.data.map(normalizeCommunityPost)));

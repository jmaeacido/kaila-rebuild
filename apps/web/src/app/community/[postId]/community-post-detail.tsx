...
export function CommunityPostDetail({
  ...
  postId: string;
  initialPost: PublicCommunityPost;
}) {
  const router = useRouter();
  const [post, setPost] = useState<CommunityPost>(() => mapPublicCommunityPost(initialPost));
  ...
  const load = useCallback(async () => {
    try {
      const [postBody, commentsBody] = await Promise.all([
        fetchJson<{ data: CommunityPost }>([
          `/api/v1/community/${postId}`,
          `/api/v1/public/community/${postId}`,
        ]),
        ...
        <div className={styles.shell}>
          <article className={styles.card}>
            <div className={styles.cardBody}>
              ...
              <p className={styles.meta}>{new Date(post.publishedAt).toLocaleString()}</p>
            </div>
          </article>
        </div>
      </main>

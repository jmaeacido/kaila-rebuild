...
export default function KatabangPage() {
  ...
  return <main className={styles.page}><header className={styles.header}><Link href="/">Back home</Link><p className={styles.eyebrow}>Katabang</p><h1>What can I help you find?</h1><p>Simple guidance through KAILA. Katabang never chooses providers, prices, or account outcomes.</p></header><section className={styles.card}><form className={styles.form} onSubmit={(event) => void ask(event)}><label>Ask Katabang<textarea value={message} maxLength={500} onChange={(event) => setMessage(event.target.value)} placeholder="How do I compare offers?" /></label><Button disabled={message.trim().length === 0 || state === "loading"}><Sparkles aria-hidden="true" />{state === "loading" ? "Finding helpâ€¦" : "Ask Katabang"}</Button></form></section>{state === "error" && <Feedback kind="error" title="Katabang is unavailable">Try again or use the main navigation.</Feedback>}{answer && <section className={styles.card} aria-live="polite"><h2>Hereâ€™s the next step</h2><p>{answer.answer}</p><div className={styles.actions}><Link className={styles.actionLink} href={answer.action.href}>{answer.action.label}</Link></div><p className={styles.meta}>{answer.disclaimer}</p></section>}</main>;
}

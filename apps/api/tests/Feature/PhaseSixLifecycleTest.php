...
class PhaseSixLifecycleTest extends TestCase
{
    ...
    public function test_work_view_exposes_only_the_current_participants_action_state(): void
    {
        ...
        $this->actingAs($provider)
            ...
            ->assertJsonPath('data.job.title', 'Fix leaking tap')
            ->assertJsonPath('data.job.description', 'The kitchen tap has a steady leak near the handle.')
            ->assertJsonPath('data.job.category.name', 'Plumbing')
            ->assertJsonPath('data.job.addressLabel', 'Near the hall')
            ->assertJsonPath('data.job.agreedAmountCentavos', 85000)
            ->assertJsonPath('data.job.agreedScope', 'Labor and standard fittings included.')
            ->assertJsonPath('data.job.counterpart.displayName', $client->name)

...
const statusLabels: Record<string, string> = {
  open: "Open",
  waiting_for_support: "Needs reply",
...
const relativeTime = (value: string) =>
  new Intl.RelativeTimeFormat("en", { numeric: "auto" }).format(
    -Math.max(1, Math.round((Date.now() - new Date(value).getTime()) / 60_000)),
    "minute",
  );
...
function SupportWorkspace() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const response = await fetch(`/api/v1/admin/marketplace/support/cases?${query}`, {
        cache: "no-store",
      });
  ...
  const openCase = useCallback(async (caseId: string) => {
    setState("loading");
    try {
      const response = await fetch(`/api/v1/admin/marketplace/support/cases/${encodeURIComponent(caseId)}`, {
        cache: "no-store",
      });
      if (!response.ok) throw new Error();
  ...
  useEffect(() => {
    ...
    const target = requestedMessageId
      ? document.getElementById(`support-message-${requestedMessageId}`)
  ...
  const counts = useMemo(
    () => ({
      total: items.length,
      urgent: items.filter((item) => ["high", "urgent"].includes(item.priority)).length,
      waiting: items.filter((item) => item.status === "waiting_for_support").length,
    }),
    [items],
  );
  ...
  async function update(data: object) {
    ...
    const response = await fetch(`/api/v1/admin/marketplace/support/cases/${selected.id}`, {
      method: "PUT",
      headers: {
  ...
  async function send(event: FormEvent) {
    ...
    setState("saving");
    const token = await prepareCsrf();
    const response = await fetch(`/api/v1/admin/marketplace/support/cases/${selected.id}/messages`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...(token ? { "X-XSRF-TOKEN": token } : {}),
  ...
  return (
    <main className={styles.page}>
      <AdminPageHeader
        ...
        description="See what needs attention, understand the customer, and resolve it in one place."
        actions={
          <Button
      ...
      <section className={styles.summary} aria-label="Support queue summary">
        <div>
      ...
      {state === "error" && (
        <div className={styles.error} role="alert">
          Support could not be updated. Check your connection and try again.
        </div>
      )}
      ...
      <div className={`${styles.workspace} ${selected ? styles.hasSelection : ""}`}>
          ...
          <div className={styles.tools}>
            ...
            <div className={styles.filters}>
              ...
              >
                ...
                <option value="waiting_for_support">Needs reply</option>
          ...
          <div className={styles.caseList}>
            ...
            {state === "ready" && items.length === 0 && (
              <div className={styles.empty}>
                ...
                <span>No requests match these filters.</span>
              </div>
            )}
            {items.map((item) => (
              <button
              ...
              >
                ...
                <span className={styles.caseCustomer}>{item.customer.name}</span>
        ...
        <section className={styles.detail} aria-label="Selected support conversation">
          {!selected ? (
            <div className={styles.noSelection}>
              <Inbox aria-hidden="true" />
              <h2>Select a conversation</h2>
              <p>Choose a request to read its history and respond.</p>
            </div>
          ) : (
            <>
              <header className={styles.detailHeader}>
                ...
                <div>
                  ...
                  <p>
                    {selected.category} · {statusLabels[selected.status] ?? selected.status}
                  </p>
                </div>
                <label>
                  Priority
              ...
              <div className={styles.customer}>
                ...
                <span>
                  {selected.assignedTo?.name ? `Owned by ${selected.assignedTo.name}` : "Unassigned"}
                </span>
              </div>
              ...
              <div className={styles.conversation} ref={conversationRef}>
                {selected.messages?.map((message) => (
                  <article
                    ...
                    key={message.id}
                    className={message.senderRole === "staff" ? styles.staffMessage : styles.customerMessage}
                    data-targeted={requestedMessageId === String(message.id) || undefined}
                  >
                    <header>
              ...
              <form className={styles.composer} onSubmit={send}>
                <label htmlFor="support-reply">Reply to {selected.customer.name}</label>
                <textarea
                  id="support-reply"
                  value={reply}
                  maxLength={4000}
                ...
                <div>
                  ...
                  <Button
                    ...
                    type="submit"
                    disabled={!reply.trim() || state === "saving" || selected.status === "closed"}
                    isLoading={state === "saving"}
                  >
                    <Send aria-hidden="true" />
                    {state === "saving" ? "Sending…" : "Send reply"}
                  </Button>
                </div>
              </form>
            </>
...
export default function SupportPage() {
  return (
    <Suspense
      fallback={
        <main className={styles.page}>
          <div className={styles.loading}>Loading support…</div>
        </main>
      }
    >
      <SupportWorkspace />

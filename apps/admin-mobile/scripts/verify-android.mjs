...
const checks = [
  ...
  [read("android/app/src/main/java/com/kaila/admin/MainActivity.java").includes("AdminNotificationChannels.ensure"), "MainActivity must create persistent notification channels before the WebView starts."],
  [read("android/app/src/main/java/com/kaila/admin/AdminNotificationChannels.java").includes('ACTIONS = "kaila_admin_actions_v2"'), "The native high-importance admin action channel must be present."],
  [read("android/app/src/main/AndroidManifest.xml").includes('android:value="kaila_admin_actions_v2"'), "FCM must fall back to the persistent admin action channel."],

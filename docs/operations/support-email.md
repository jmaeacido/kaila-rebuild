...
KAILA sends transactional mail from **`no-reply@kaila-app.com`** (Resend).  
User-facing help uses **`support@kaila-app.com`**, which must **receive** mail.

Resend is configured for **sending only**. Use a free forwarder for inbound support mail.

## Recommended: ImprovMX (free forwarding)

DNS for `kaila-app.com` is managed separately from Resend sending records.
...
> **Note:** Root-domain MX records route *all* inbound mail for `@kaila-app.com` through ImprovMX. That is fine while Resend handles outbound only. You can add more aliases later (`hello@`, `billing@`, etc.).
...
Confirm `kaila-app.com` is verified in the Resend dashboard (same domain already used by pikl). Keep SPF/DKIM records Resend provides on the domain.
...
3. Confirm to queue branded invites through Resend.
